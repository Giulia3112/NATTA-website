...
const opportunityUpdateSchema = z.object({
  ...
  opportunityType: z.enum(["Scholarship", "Fellowship", "Accelerator", "Incubator", "Competition", "Internship", "Grant", "Conference", "Exchange Program", "Course", "Other"]),
  stage: z.enum(["High school", "Undergraduate", "Graduate", "Startup idea", "MVP", "Revenue", "Scale", "Multi-stage", "Other"]),
    ...
    try {
      ...
      for (const opp of unique) {
        ...
        await db.insert(pendingOpportunities).values({
          ...
          stage: opp.stage,
          regions: (Array.isArray(opp.regions) && opp.regions.length > 0) ? opp.regions : ["Global"],
          mode: opp.mode,
          fields: opp.fields,
          funding: opp.funding,
  ...
  approve: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await db.insert(opportunities).values({
        ...
        stage: data.stage,
        regions: (Array.isArray(data.regions) && data.regions.length > 0) ? data.regions : (Array.isArray(pending.regions) && pending.regions.length > 0) ? pending.regions : ["Global"],
        mode: data.mode,
        fields: data.fields,
